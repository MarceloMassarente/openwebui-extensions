...
- **� SDK & CLI 升级**: 从 `github-copilot-sdk` v0.1.30 迁移到 **v0.2.2**，全面兼容新的 dataclass API 和 positional `send()` 参数。
- **🗂️ 模型过滤与排序**: 新增提供商感知排序（OpenAI → Anthropic → 其他）和精确前缀模型过滤，干净移除已下线模型。
...
- **GitHub Copilot SDK** (`github-copilot-sdk`): 深度集成 GitHub Copilot SDK 的强大 Agent (v0.12.3)。支持高性能进程池优化、纯 BYOK 模式、智能意图识别、自主网页搜索与上下文压缩。
