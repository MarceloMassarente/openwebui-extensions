...
**核心设计**：每个 Agent 由其 OpenWebUI **模型标签、系统提示词和模型描述**（`meta.system_prompt`）定义——不依赖基础模型 ID。Copilot SDK 读取模型标签、系统提示词和模型描述来注册每个子 Agent。只需在 OpenWebUI 中配置好模型，三者自动生效，无需额外配置。

比如：一个数据分析团队，在 OpenWebUI 中配置三个自定义模型，共享同一标签 `data-agent`：
- **Agent 1**（标签：`data-agent`，模型 A）：描述 = "首席数据分析师，统筹协调团队工作"；系统提示词 = "你是一个首席数据分析师，统筹协调团队工作"
- **Agent 2**（标签：`data-agent`，模型 B）：描述 = "数据处理与统计分析专家，擅长用 Python/pandas 处理海量数据"；系统提示词 = "你专注于数据处理和统计分析，擅长用 Python/pandas 处理海量数据"
- **Agent 3**（标签：`data-agent`，模型 C）：描述 = "数据可视化和报告生成专家，擅长制作图表和撰写分析结论"；系统提示词 = "你专注于数据可视化和报告生成，擅长制作图表和撰写分析结论"

用户说"分析这份销售数据"，Agent 1 作为主 Agent 统筹工作：可能**按需分发给 Agent 2 和 Agent 3**，也可能自己承担部分工作。子 Agent 完成后，Agent 1 收集汇总结果，并可能继续收尾，最终将完整任务结果返回给用户。
