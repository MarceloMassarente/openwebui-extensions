...
对于带版本号的插件更新，在创建 PR 之前，贡献者或维护者可以使用或参考 `.github/skills/release-prep/SKILL.md`。

- 使用 `release-prep` 同步代码与文档所需发布面的版本变更
- 使用它生成双语版本发布说明，并草拟本次发布准备的 Conventional Commit 提交信息
- 该技能不会执行 `git push` 或创建 PR，因此准备完成后还需要继续正常的 PR 流程
